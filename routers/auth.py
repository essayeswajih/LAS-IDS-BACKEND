...
def get_key():
    return ''.join(random.choices(string.ascii_letters + string.digits, k=random.randint(150, 200)))
...
def generate_unique_username(db: db_dependency, first_name, last_name):
    base_username = f"{first_name}.{last_name}_{random.randint(0, 999)}"
    while db.query(User).filter(User.username == base_username).first():
        base_username = f"{first_name}.{last_name}_{random.randint(0, 999)}"
    return base_username

def validate_user_email(db: db_dependency, email):
